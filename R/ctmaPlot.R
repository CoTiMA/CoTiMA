  ...
  { # some checks
    ...
    if (n.fitted.obj != 1) {
      ...
      if (is.null(tmp)) { # CHD 27.2.2023
        activeDirectory <- ctmaFitObject$studyFitList[[1]]$activeDirectory
        if (is.null(activeDirectory)) activeDirectory <- ctmaFitObject$studyFitList[[1]]$argumentListactiveDirectory
      } else {
          activeDirectory <- tmp
        }
    }
