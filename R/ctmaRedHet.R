...
{  # begin function definition (until end of file)
  ...
  ## extract model params ctmaFit #######################################################################
  {
    ...
    if ((is.null(ctmaFitObjectMod$argumentList$invariantDrift)) |
        (length(ctmaFitObjectMod$argumentList$invariantDrift) == n.latent1^2) ){
      if (activateRPB==TRUE) {RPushbullet::pbPost("note", paste0("CoTiMA (",Sys.time(),")" ), paste0(Sys.info()[[4]], "\n","Attention!"))}
    ...
    if ((is.null(ctmaFitObject$argumentList$invariantDrift)) |
      ...
      if (activateRPB==TRUE) {RPushbullet::pbPost("note", paste0("CoTiMA (",Sys.time(),")" ), paste0(Sys.info()[[4]], "\n","Attention!"))}
      Msg <- "The ctmaFitObject provided war created with not a single drift parameter varying across primary study.
            Reduction in heterogeneity of drift effects cannot be computed because there is no heterogeneity."
      message(Msg)
    }
    ...
    modDriftList1 <- list()
    modDriftList1[[1]] <- ctmaFitObject$studyFitList$stanfit$rawposterior[,referenceTargetCols1]; modDriftList1[[1]][1,] # will be tformed last
    ...
    tmp2 <- which(!(is.na(ctmaFitObject$studyFitList$ctstanmodelbase$pars$param))); tmp2
    pars2 <- ctmaFitObject$studyFitList$ctstanmodelbase$pars[tmp1, ]; pars2
    ### determine the positions where the study_dummy moderators of the drift effects are located #####
    ...
    ### extract sampled effects #######
    modDriftList2 <- list()
    modDriftList2[[1]] <- ctmaFitObjectMod$studyFitList$stanfit$rawposterior[,referenceTargetCols2]; modDriftList2[[1]][1,] # will be tformed last
    TIpredEffectsList2 <- list()
    e2 <- ctsem::ctExtract(ctmaFitObjectMod$studyFitList)
    for (s in 2:n.studies2) TIpredEffectsList2[[s-1]] <- e2$TIPREDEFFECT[,referenceTargetCols1, (s-1)]
    ...
    # get effects from ctmaFit without moderators #########################################################
    {
      ...
      modDriftMean1[[1]] <- matrix(NA, n.latent1, n.latent1)
      modDriftSD1[[1]] <- matrix(NA, n.latent1, n.latent1)
      counter <- 0
      for (l in 1:(n.latent1)) {
        for (m in 1:(n.latent1)) {
          ...
          for (f in 1:finishsamples1) {
            param <- matrix(modDriftList1[[1]][f, ], n.latent1, n.latent1, byrow=T)
            param <- param[l,m]; param
            transformedParam <- c(transformedParam, eval(parse(text=transforms1[counter]))); transformedParam
          }
          modDriftMean1[[1]][l,m] <- mean(transformedParam)
          modDriftSD1[[1]][l,m] <- sd(transformedParam)
          # undoTimScaling after tform
          if (undoTimeScaling & (!(is.null(ctmaFitObject$summary$scaledTime)) ) ) {
            modDriftMean1[[1]][l,m] <- modDriftMean1[[1]][l,m] * ctmaFitObject$summary$scaledTime
            modDriftSD1[[1]][l,m] <- modDriftSD1[[1]][l,m] * ctmaFitObject$summary$scaledTime
          }
        }
      }
      ## apply tforms to other groups
      for (k in 2:n.studies1) {
        counter <- 0
        ...
        for (l in 1:(n.latent1)) {
          for (m in 1:(n.latent1)) {
            ...
            for (f in 1:finishsamples1) {
              #f <- 1
              tmp1 <- matrix(modDriftList1[[1]][f, ], n.latent1, n.latent1, byrow=T); tmp1
              tmp2 <- matrix(TIpredEffectsList1[[k-1]][f, ], n.latent1, n.latent1, byrow=T); tmp2
      ...
      allDriftsMean1 <- rbind(allDriftsMean1[-1,], allDriftsMean1[1, ]); allDriftsMean1
      allDriftsSD1 <- matrix(unlist(modDriftSD1), ncol=n.latent1^2, byrow=T); allDriftsSD1
      allDriftsSD1 <- rbind(allDriftsSD1[-1,], allDriftsSD1[1, ]); allDriftsSD1
    }
    ...
    # get effects from ctmaFitMod with moderators #########################################################
    {
      ...
      modDriftMean2[[1]] <- matrix(NA, n.latent2, n.latent2)
      modDriftSD2[[1]] <- matrix(NA, n.latent2, n.latent2)
      counter <- 0
      for (l in 1:(n.latent2)) {
        for (m in 1:(n.latent2)) {
          ...
          for (f in 1:finishsamples2) {
            param <- matrix(modDriftList2[[1]][f, ], n.latent2, n.latent2, byrow=T)
            param <- param[l,m]; param
            transformedParam <- c(transformedParam, eval(parse(text=transforms2[counter]))); transformedParam
          }
          modDriftMean2[[1]][l,m] <- mean(transformedParam)
          modDriftSD2[[1]][l,m] <- sd(transformedParam)
          # undoTimScaling after tform
          if (undoTimeScaling & (!(is.null(ctmaFitObjectMod$summary$scaledTime)) ) ) {
            modDriftMean2[[1]][l,m] <- modDriftMean2[[1]][l,m] * ctmaFitObjectMod$summary$scaledTime
            modDriftSD2[[1]][l,m] <- modDriftSD2[[1]][l,m] * ctmaFitObjectMod$summary$scaledTime
          }
        }
      }
      ## apply tforms to other groups ######
      for (k in 2:n.studies2) {
        counter <- 0
        ...
        for (l in 1:(n.latent2)) {
          for (m in 1:(n.latent2)) {
            ...
            for (f in 1:finishsamples2) {
              #f <- 1
              tmp1 <- matrix(modDriftList2[[1]][f, ], n.latent2, n.latent2, byrow=T); tmp1
              tmp2 <- matrix(TIpredEffectsList2[[k-1]][f, ], n.latent2, n.latent2, byrow=T); tmp2
              param <- tmp1[l,m] +tmp2[l,m]; param
              transformedParam <- c(transformedParam, eval(parse(text=transforms2[counter]))); transformedParam
            }
      ...
      allDriftsMean2<- matrix(unlist(modDriftMean2), ncol=n.latent2^2, byrow=T); allDriftsMean2
      allDriftsMean2 <- rbind(allDriftsMean2[-1,], allDriftsMean2[1, ]); allDriftsMean2
      allDriftsSD2 <- matrix(unlist(modDriftSD2), ncol=n.latent2^2, byrow=T); allDriftsSD2
      allDriftsSD2 <- rbind(allDriftsSD2[-1,], allDriftsSD2[1, ]); allDriftsSD2
    }
  }

  # 'translate' objects into objects as used in ctmaBiG #####
  {
    names11 <- names(ctmaFitObject$modelResults$DRIFT); names11
    names12 <- names(ctmaFitObject$modelResults$DIFFUSION); names12
    names13 <- names(ctmaFitObject$modelResults$T0VAR); names13
    ...
    colnames(all_SE1) <- colnames(all_Coeff1) <- colnames(all_SE1) <- colnames(all_Coeff1) <- c(names11)
  ...
  ## FIXED EFFECTS ANALYSIS ###############################################################################
  {
    ...
    MeanOfDriftValues1 <- DriftMeans1
    MeanOfDriftValues2 <- DriftMeans2

    newNames <- names(FixedEffect_Drift1) ; newNames
  ...
  {
    Ttot_DriftWeights1 <- Ttot_DriftWeights2 <- 0
    Ttot_DriftMeans1 <- Ttot_DriftMeans2 <- 0
