...
ctmaCompFit <- function(model1=NULL, model2=NULL) {
  #model1 <- CoTiMAMultipleStanctFit
  #model2 <- CoTiMAEqualStanctFit
  ...
  for (i in 1:n.model1) {
    ...
    for (j in 1:n.model2) {
      ...
      allResults[[counter]]$"Diff_Minus2LL: " <- abs((model1$summary$minus2ll[[i]] - model2$summary$minus2ll[[j]])); allResults[[counter]]$Diff_Minus2LL
      allResults[[counter]]$"Diff_df: " <- abs((model1$summary$df[[i]] - model2$summary$df[[j]])); allResults[[counter]]$Diff_df
