                     ...
                     excludedStudies=NULL)
{
  ...
  if ( (!is.null(selectedStudies)) & (!(is.null(ctmaPrepObject))) )  {
    ErrorMsg <- "The arguments \"selectedStudies\" and \"ctmaPrepObject\" cannot be used together! \nGood luck for the next try!"
    stop(ErrorMsg)
  }
  ...
  if ( (!is.null(ctmaPrepObject)) & (!is.null(excludedStudies)) )  {
    ...
    for (i in sort(excludedStudies, decreasing=TRUE)) {
      for (j in 1:length(names(ctmaPrepObject))) {
        if (names(ctmaPrepObject)[j] %in% c("studyNumbers", "empcovs", "deltas", "sampleSizes",
          ...
          ctmaPrepObject[[j]][[i]] <- NULL
        } else {
          if (length(ctmaPrepObject[[j]]) >= old.n.studies) ctmaPrepObject[[j]][[i]] <- NULL
        }
      }
    }
    ...
    if (summary == TRUE) {
      ...
      if (length(existExmpcov) > 0) {
        ...
        #maxEmpcov <- 'currently disabled'
        n.variables <- maxEmpcov/maxWaves; n.variables
        #n.variables <- 'currently disabled'
      }
