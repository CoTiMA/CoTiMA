...
ctmaShapeRawData <- function(
  ...
  if (standardization == "withintimeb") {
    Msg <- "Variables are standardized within time points. All cases are retained. The user is adviced to use 'sameInitialTimes=TRUE' in ctsem or CoTiMA applications.\n"
    message(Msg)
    for (c in allOutputVariablesNames) {
      tmpData[, c] <- scale(tmpData[, c])
    }
