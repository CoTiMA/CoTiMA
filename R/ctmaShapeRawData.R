...
#' @param standardization the way to standardize possible raw ("none", "withinTime", "withinColumn", "withinPerson", or "overall"). Only applies if the list for specifying raw data information contains the list element 'standardize=TRUE'
#' @param keepTimePoints Do not fill any gaps in the data (no left shift).
...
#' @param experimental FALSE (default) or TRUE. Deprecated.
#'
#' @examples
#' \dontrun{
...
ctmaShapeRawData <- function(
  ...
  # some checks
  {
    ...
    if (!( standardization %in% c("none", "withintime", "withincolumn", "withinperson", "overall"))) {
      ErrorMsg <- "\nThe standardization argument hast to be one out of c(\"none\", \"withintime\", \"withincolumn\", \"withinperson\", \"overall\"))). \nGood luck for the next try!"
      stop(ErrorMsg)
    }
