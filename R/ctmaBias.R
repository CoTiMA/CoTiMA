...
ctmaBias <- function(
  ...
  # probability level (condition) below which to swith from PET to PEESE (Stanley, 2017, SPPS,p. 582, below Eq. 2)
  PETPEESEalpha=.10,
...
{  # begin function definition (until end of file)
  ...
  if (resultsFilePrefix=="ctmaBias") {
    if (activateRPB==TRUE) {RPushbullet::pbPost("note", paste0("CoTiMA (",Sys.time(),")" ), paste0(Sys.info()[[4]], "\n","Data processing stopped.\nYour attention is required."))}
    cat("The default results file prefix (ctmaBias) has been chosen.", "\n")
...

  {
    ...
    for (i in 1:n.studies) {
      ...
      tmp2  <- tmp1[grep("diffusion", names(tmp1))]; tmp2
      DIFFUSIONCoeff[i, ] <- ctmaBase2Full(OpenMx::vech2full(tmp2)); DIFFUSIONCoeff
      tmp2  <- tmp1[grep("T0var", names(tmp1))]; tmp2
      T0VARCoeff[i, ] <- ctmaBase2Full(OpenMx::vech2full(tmp2)); T0VARCoeff

      tmp1 <- all_SE[[i]]; tmp1
      DRIFTSE[i, ] <- tmp1[grep("toV", rownames(tmp1))]; DRIFTSE
      tmp2  <- tmp1[grep("diffusion", rownames(tmp1))]; tmp2
    ...
    tmp2 <- c(OpenMx::vech2full(names(all_Coeff[[1]][grep("diffusion", names(all_Coeff[[1]]))]))); tmp2
    ...
    #allStudiesDRIFT_effects <- matrix(t(cbind(unlist(DRIFTCoeff), unlist(DRIFTSE)) ), n.studies, 2*n.latent^2, byrow=T); allStudiesDRIFT_effects
