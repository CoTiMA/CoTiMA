debug <- 0
if (debug == 1) {
  empCovMat <- currentEmpcov
  empN <- currentSampleSize
...
{  # begin function definition (until end of file)
  ...
  if (!(isSymmetric(empCovMat))) {
    if (activateRPB==TRUE) {pbPost("note", paste0("CoTiMA (",Sys.time(),")" ), paste0(Sys.info()[[4]], "\n","Data processing stopped.\nYour attention is required."))}
    cat(red$bold("The empirical covariance matrix provided is not symmetrix!", sep="\n")) 
    stop("Good luck for the next try!")
  }
  ...
  #if ( (is.null(empNMat) & is.null(empN)) | ((!(is.null(empNMat)) & (!(is.null(empN))))) ) {
  ...
  if (!("MASS" %in% (.packages()))) library(crayon)  
  if("psych" %in% rownames(installed.packages()) == FALSE) {install.packages("psych")} 
  if (!("psych" %in% (.packages()))) library(crayon)  
  ...
  eigenValueProlems <- "Congratulations! This covariance matrix is positive definite (no negative eigenvalues)."
  ...
  if (dim(k)[1] > 0) {
    ...
    randomCors <- rnorm(length(uniqueK)/2, 0, .01); randomCors
  ...
  while(any(eigen(newMat)$values < 0)) {
    ...
    eigenValueProlems <- paste0("The correlation matrix of Study ", i, "had negative Eigenvalues. I tried to fix this, but better check the matrix.")
  }
  
  toBeMadeNA <- makeNAdueToNACors <- makeNAdueToZeroN <- NULL
  # store rows and cols with correlations == NA
  #if (dim(k)[1] > 0) toBeMadeNA <- which(table(c(k[,2])) > dim(newMat)[2]/2); toBeMadeNA # which columns in the final data set should be made NA
  ...
  #tmp <- which((apply(currentN, 1, sum) == 0) & (apply(currentN, 2, sum) == 0) == TRUE); tmp
  makeNAdueToZeroN <- which((apply(currentN, 1, sum) == 0) & (apply(currentN, 2, sum) == 0) == TRUE); makeNAdueToZeroN
  #if (length(tmp) > 0) toBeMadeNA <- c(toBeMadeNA, tmp)
  ...
  ###### Pairwise deletion ######
  matrixL <- lapply(1:(2^(dim(currentN)[1])), list); matrixL
  ...
  while(length(matrixL[[whileCounter]]) > 1) {
    #if (whileCounter == 9) break
    matrixL[[whileCounter]]
    ...
    if ( (minN == 1 & matDim == 1) | (minN == 0) ) {
      ...
      if (!(minN == 1 & matDim == 1)) {
        ...
        # generate data
        matrixL[[whileCounter]]$data <- mvrnorm(n=minN, mu=rep(0, matDim), Sigma=r, empirical = TRUE); matrixL[[whileCounter]]$data
      ...
      if (matDim > 1) { 
        ...
        if (is.null(dim(matrixL[[matCounter + 1]]$n))) {
          matrixL[[matCounter + 1]]$n <- matrix(matrixL[[matCounter + 1]]$n, 1, 1, 
                                                dimnames=list(unlist(dimnames(r)[1])[-minNcoord[1]], unlist(dimnames(r)[1])[-minNcoord[1]]))
          # tryout: prevent too large diagonal N: matrixL[[matCounter + 1]]$n <- matrixL[[matCounter + 1]]$n - 1
        }
        if (is.null(dim(matrixL[[matCounter + 2]]$n))) {
          matrixL[[matCounter + 2]]$n <- matrix(matrixL[[matCounter + 2]]$n, 1, 1, 
                                                dimnames=list(unlist(dimnames(r)[1])[-minNcoord[2]], unlist(dimnames(r)[1])[-minNcoord[2]]))
          # tryout: prevent too large diagonal N: matrixL[[matCounter + 2]]$n <- matrixL[[matCounter + 2]]$n - 1
        }
  ...
  nDiff <- corr.test(newData, ci=FALSE)$n - origN; nDiff
  rDiff <- round(corr.test(newData, ci=FALSE)$r - currentR, 6); rDiff
  currentData <- list()
  counter <- 0
  for (i in 1:(dim(nDiff)[1])) {
    if (nDiff[i,i] < -1) {
      counter <- counter +1
      currentData[[counter]] <- mvrnorm(n=-nDiff[i,i], mu=rep(0), Sigma=matrix(currentR[i,i], 1, 1), empirical = TRUE)
  ...
  results <- list(newData, eigenValueProlems, nDiff, overallNDiff, relativeNDiff)
